"""
Tolerant comparator for parity_multi_runtime_2d_modes.

Same tolerance budget as the 2D density-only parity (psi atol 5e-13,
scalar atol 1e-10, integer fields exact).
"""
import math, subprocess, sys
from pathlib import Path

REPO = Path(__file__).resolve().parent.parent.parent.parent
NATIVE = REPO / "native" / "c" / "parity" / "parity_multi_runtime_2d_modes"
PY = REPO / "src" / "scripts" / "codegen" / "multi_runtime_2d_modes_to_text.py"
...
def floats_close(a, b, atol, rtol):
    ...
    if abs(a - b) <= atol: return True
    scale = max(abs(a), abs(b))
    if scale > 0 and abs(a - b) / scale <= rtol: return True
    return False

def compare_line(py_line, c_line):
    if py_line == c_line: return True, ""
    py_toks = py_line.split(); c_toks = c_line.split()
    ...
    for i, (a, b) in enumerate(zip(py_toks, c_toks)):
        ...
        if not ok:
            return False, f"float mismatch at tok {i}: {a} vs {b} (abs={abs(fa-fb):.3e})"
    return True, ""

def main():
    py_out = subprocess.run(["python3", str(PY)], capture_output=True, text=True, check=True).stdout.splitlines()
    c_out = subprocess.run([str(NATIVE)], capture_output=True, text=True, check=True).stdout.splitlines()
    ...
    for i, (a, b) in enumerate(zip(py_out, c_out)):
        ...
        if ok: passes += 1
        else: fails.append(f"  L{i+1}: {why}\n    py: {a[:200]}\n    c : {b[:200]}")
    total = len(py_out)
    if fails:
        print(f"FAIL: {len(fails)}/{total} lines diverge")
        for f in fails[:10]: print(f)
        return 1
    print(f"parity_multi_runtime_2d_modes: PASS ({passes}/{total})")
    return 0
