# Stdlib — módulos simples

Importe com `import nome;`. Assinaturas completas; nada escondido.
...
random.randint(1, 10)  // int, ambos os fins incluídos
...
// encode/decode são apelidos de stringify/parse
```
...
datetime.timestamp()  // segundos epoch
datetime.year()  datetime.month()  datetime.day()

import regex;
regex.match("[0-9]+", "abc123")     // false (match = do início)
regex.search("[0-9]+", "abc123")    // "123"
regex.findall("[0-9]", "a1b22")     // ["1", "2", "2"]
regex.replace("[0-9]", "#", "a1b")  // "a#b"

// plot: jeitos matplotlib (triad.plot é o mesmo módulo)
import plot;
plot.plot([0, 1, 2], [0, 1, 4]);
plot.title("parábola");
plot.savefig("saida.png");
import io;
io.print("x");   // print como função (io.input lê stdin)
```
