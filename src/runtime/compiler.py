...
from runtime.core.equation_runtime import EquationRuntime, ReadoutConfig
...
class TriadCompiler:
    ...
    def _compile_sat(self, n_vars: int=3, n_clauses: int=9, **kwargs) -> CompiledProgram:
        N = max(64, n_vars * 8)
        return CompiledProgram(substrates=[SubstrateConfig(name='sat_0', N=N, Lambda=-5.0, alpha=0.1, sigma=1.7, V_ext=None, nu=(2.0, 0.5), lam=(-0.5, -0.2))], couplings=[], readout=ReadoutConfig(), T=10.0, metadata={'task': 'sat', 'n_vars': n_vars, 'n_clauses': n_clauses})
