    ...
    def _compile_loop(self, blk: LoopBlock):
        if isinstance(blk.target, NumLit):
            ...
            for i in range(effective_n):
                ...
                iter_seg_ranges.append((start_seg, end_seg))
    ...
    def _compile_if(self, blk: IfBlock):
        ...
        def make_modulator(invert: bool):
            ...
            def mod(rho_now: dict) -> float:
                ...
                if sub is None:
                    return 1.0
                from runtime.physics.observables import crystallinity as _cr
        ...
        if isinstance(entry, str):
            ...
            for m in metrics:
                if m == 'k_star':
                    ...
                    from runtime.physics.observables_atoms import atom_count_nd
                    row[m] = int(atom_count_nd(sub.psi, sub.dx))
                elif m == 'atom_centroids':
                    from runtime.physics.observables_atoms import atom_centroids_nd
                    row[m] = atom_centroids_nd(sub.psi, sub.dx).tolist()
                elif m == 'atom_separation':
                    if D == 1:
                        from runtime.physics.observables_atoms import atom_separation as _asep
                        row[m] = float(_asep(sub.psi, sub.dx))
                    else:
                        row[m] = float('nan')
                elif m == 'atoms_per_region':
                    from runtime.physics.observables_atoms import atoms_per_region
                    row[m] = float(atoms_per_region(sub.psi, sub.dx))
                elif m == 'atomicity_ratio':
                    from runtime.physics.observables_atoms import atomicity_ratio as _ar
                    ...
                    if D == 1 and sub.density_traj is not None:
                        from runtime.physics.observables_atoms import atom_persistence_late as _apl
                        row[m] = float(_apl(sub.density_traj, sub.dx))
                    else:
                        row[m] = float('nan')
                elif m == 'bravais_family':
                    if D == 3:
                        from runtime.codec.codec_3d import bravais_family as _bf
                    ...
                    from runtime.physics.consciousness import lzc_of_trajectory
                    if sub.density_traj is not None:
                        row[m] = float(lzc_of_trajectory(sub.density_traj))
                    else:
                        row[m] = float('nan')
                elif m == 'phi_id':
                    from runtime.physics.consciousness import phi_id_proxy
                    if sub.density_traj is not None:
                        row[m] = float(phi_id_proxy(sub.density_traj))
                    else:
                        row[m] = float('nan')
                elif m == 'causal_density':
                    from runtime.physics.consciousness import causal_density_pair
