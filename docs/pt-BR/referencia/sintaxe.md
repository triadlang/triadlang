# Sintaxe — a linguagem inteira numa página

## Frases e valores

```tri-frag
let x = 10;          const K = 2;          // mutável / congelada
10  3.14  "texto"  true  false  none      // Int Float String Bool none
"a" + "b"  2 ** 10  7 % 3                 // cola, potência, módulo
f"x={x} {x * 2}"                          // f-string calcula dentro
str(42)  type(x)  len([1])                // converte / revela / conta
```
...
for v in [1, 2] { }                       // percorre o percorrível
while ok { }  break;  continue;           // trio clássico
match v { case 1 { } case 2 { } }         // escolhe por valor
```

## Funções e erros

```tri-frag
fn f(a, b=2, *args, **kw) { return a; }   // padrão + sobra
yield v;                                  // pausa-e-entrega (em fn)
try { } catch e { } finally { }           // arrisca / cai / sempre
throw "motivo";  assert ok;               // lança / trava
async fn f() { }  await g();              // await só dentro de async
```
...
type P { nome: String; idade: Int; }      // molde struct
class C { fn __init__(self) {} }          // molde comportamento
class D inherits C { }                    // reaproveita + varia
```

## Módulos

```tri
import math;  import math as m;           // inteiro / apelido
from math import sqrt, pi;                // escolhidos
import triad.ntri as np;                  // família triad.* (39)
import numpy;  import flask;               // Python real (lista safe)
```

## DSL física (mesmo runner)
