# Stdlib — enkla moduler

Importera med `import namn;`. Fullständiga signaturer; inget gömt.
...
import random;
random.seed(0)
random.random()        // flyttal 0..1
random.randint(1, 10)  // int, båda ändar inklusive
...
// encode/decode är alias för stringify/parse
```
...
import regex;
regex.match("[0-9]+", "abc123")     // false (match = från start)
regex.search("[0-9]+", "abc123")    // "123"
regex.findall("[0-9]", "a1b22")     // ["1", "2", "2"]
regex.replace("[0-9]", "#", "a1b")  // "a#b"

// plot: matplotlib-manér (triad.plot är samma modul)
import plot;
plot.plot([0, 1, 2], [0, 1, 4]);
plot.title("parabel");
plot.savefig("ut.png");
import io;
io.print("x");   // print som funktion (io.input läser stdin)
```
