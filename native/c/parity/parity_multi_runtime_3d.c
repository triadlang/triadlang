/*
 * parity_multi_runtime_3d.c — Native fingerprint of TriadMultiRuntime (D=3).
 *
 * Two 3D substrates (N=8, L=8) coupled bidirectionally via density mode,
 * P1+P2+P3 all live, f_FDT=0. Mirrors scripts/multi_runtime_3d_to_text.py
 * exactly.
 */
...
int main(void) {
    ...
    const double T_settle = 0.2;
    const double T_couple = 0.3;
    ...
    int idA = triad_mr_add_substrate(rt, "A", /*D*/3,
        N, L, 1.0, 1.0, 1.0,
        /*Lambda*/-0.2, 0.15, 1.5,
        /*Gamma*/0.05, 0.0,
        1, nu, lam,
        2, 11, "harmonic", NULL);
    int idB = triad_mr_add_substrate(rt, "B", /*D*/3,
        N, L, 1.0, 1.0, 1.0,
        -0.2, 0.15, 1.5,
        0.05, 0.0,
        1, nu, lam,
    ...
    for (int n_i = 0; n_i < 2; ++n_i) {
        ...
        if (s->n_records > 0) {
            /* Same transposition logic as the 2D parity: Python iterates
             * (*grid_shape, n_records) in flat-C, so for 3D that's
             *   for i in N: for j in N: for k in N: for r in n_records.
             * We re-pack to that order before checksumming. */
