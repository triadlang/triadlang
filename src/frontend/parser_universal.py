...
class Parser:
    ...
    def __init__(self, tokens: list[Token], file: str='', source: str=''):
        self.toks = tokens
        self.i = 0
        self.file = file
        self.source = source
    ...
    def _expect(self, kind, value=None) -> Token:
        ...
        if t.kind != kind or (value is not None and t.value != value):
            ...
            raise ParseError(f'expected {exp}, got {t.kind} {t.value!r}', t.line, t.col, self.file, self.source)
        return self._eat()
    ...
    def _parse_stmt(self) -> Stmt:
        ...
        if t.kind == 'SYMBOL' and t.value == '@':
            next_t = self._peek(1)
            _TRIAD_ANNOTATION_KEYS = ('T', 'D', 'N', 'dt', 'par', 'gpu')
            if next_t.kind in ('IDENT', 'KEYWORD') and next_t.value in _TRIAD_ANNOTATION_KEYS:
    ...
    def _parse_fn(self, async_=False) -> FnDecl:
        p = self._pos()
        decorators = None
        if self._pending_decorators:
            decorators = list(self._pending_decorators)
        ...
        return FnDecl(name=name, params=params, return_type=ret, body=body, is_async=async_, decorators=decorators, pos=p)
    ...
    def _parse_import(self) -> ImportStmt:
        ...
        if self._at('KEYWORD', 'as'):
            self._eat()
            t = self._peek()
            alias = self._eat().value if t.kind in ('IDENT', 'KEYWORD') else self._expect('IDENT').value
        self._eat_semi()
    ...
    def _parse_from_import(self) -> FromImportStmt:
        ...
        self._eat()
        t = self._peek()
        path = [self._eat().value if t.kind in ('IDENT', 'KEYWORD') else self._expect('IDENT').value]
        while self._at('SYMBOL', '.'):
            self._eat()
            t = self._peek()
            path.append(self._eat().value if t.kind in ('IDENT', 'KEYWORD') else self._expect('IDENT').value)
        self._expect_kw('import')
        t = self._peek()
        names = [self._eat().value if t.kind in ('IDENT', 'KEYWORD') else self._expect('IDENT').value]
        aliases = [None]
        if self._at('KEYWORD', 'as'):
            self._eat()
            t2 = self._peek()
            aliases[0] = self._eat().value if t2.kind in ('IDENT', 'KEYWORD') else self._expect('IDENT').value
        while self._at('SYMBOL', ','):
            self._eat()
            t = self._peek()
            names.append(self._eat().value if t.kind in ('IDENT', 'KEYWORD') else self._expect('IDENT').value)
            a = None
            if self._at('KEYWORD', 'as'):
                self._eat()
                t2 = self._peek()
                a = self._eat().value if t2.kind in ('IDENT', 'KEYWORD') else self._expect('IDENT').value
            aliases.append(a)
        self._eat_semi()
        return FromImportStmt(path=path, names=names, aliases=aliases, pos=p)
    ...
    def _parse_decorator_expr(self):
        ...
        while True:
            if self._at('SYMBOL', '.'):
                self._eat()
                t = self._peek()
                field_name = self._eat().value if t.kind in ('IDENT', 'KEYWORD') else self._expect('IDENT').value
                if self._at('SYMBOL', '('):
                    self._eat()
                    args, kwargs = self._parse_call_args()
                    self._expect_sym(')')
                    expr = MethodCallExpr(obj=expr, method=field_name, args=args, kwargs=kwargs, pos=p)
                else:
                    expr = FieldExpr(obj=expr, field=field_name, pos=p)
            elif self._at('SYMBOL', '('):
                self._eat()
                args, kwargs = self._parse_call_args()
                self._expect_sym(')')
                expr = CallExpr(func=expr, args=args, kwargs=kwargs, pos=p)
            else:
                break
        self._eat_semi()
        return expr
                ...
                if self._at('SYMBOL', ':'):
                    ...
                    if self._at('SYMBOL', ':'):
                        ...
                        if self._at('SYMBOL', ','):
                            ...
                            self._expect_sym(']')
                            expr = IndexExpr(obj=expr, index=TupleExpr(elements=elements, pos=p), pos=expr.pos if hasattr(expr, 'pos') else p)
                        else:
                            self._expect_sym(']')
    ...
    def _parse_primary(self) -> Expr:
        ...
        if t.kind == 'BYTES':
            self._eat()
            from frontend.ast_nodes import BytesLit
            return BytesLit(t.value, p)
        if t.kind == 'FSTRING':
            ...
            for part in t.value:
                if part[0] == 'str':
                    parsed_parts.append(('str', part[1]))
                else:
                    from frontend.lexer_universal import tokenize
                    expr_str = part[1]
                    fmt_spec = part[2] if len(part) > 2 else None
                    sub_toks = tokenize(expr_str, '<fstring>')
                    sub_parser = Parser(sub_toks, '<fstring>')
                    sub_expr = sub_parser._parse_expr()
                    parsed_parts.append(('expr', sub_expr, fmt_spec))
        ...
        if t.kind == 'IDENT':
            if t.value == 'lambda':
                return self._parse_py_lambda()
            self._eat()
            return Ident(t.value, p)
        if t.kind == 'KEYWORD' and self._peek(1).kind == 'SYMBOL' and self._peek(1).value == '(':
            self._eat()
            return Ident(t.value, p)
        if t.kind == 'KEYWORD' and self._peek(1).kind == 'SYMBOL' and self._peek(1).value in ('=', ';', '\n', ')', ']', ','):
            self._eat()
            return Ident(t.value, p)
        if t.kind == 'SYMBOL' and t.value == '(':
            self._eat()
    ...
    def _parse_py_lambda(self) -> LambdaExpr:
        from frontend.ast_nodes import Param
        p = self._pos()
        self._eat()
        params = []
        while not self._at('SYMBOL', ':'):
            ...
            if t is None:
                break
            if t.kind in ('IDENT', 'KEYWORD'):
                params.append(Param(name=t.value, type_ann=None, default=None))
            if self._at('SYMBOL', ','):
                self._eat()
        self._expect_sym(':')
        expr = self._parse_expr()
        from frontend.ast_nodes import ReturnStmt
