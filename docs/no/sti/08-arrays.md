# 08 — Arrayer: tenk i vektorer

## `triad.ntri`: numpy-manerer, triad-aksent
...
Én operator, hele arrayet. Ingen løkke i sikte.

## Broadcasting: skalar møter vektor
...
Skalaren strekker seg over hver plass.

## Byggere og reduksjoner
...
// vektor: veien for tusentalls og oppover
let big = np.ones(1000000);
print(np.sum(big));   // array(1000000.0, dtype=float64)
```

Tommelfingerregel: uttrykk matten, ikke vandringen.

✎ prøv: `sin` av `linspace(0, 3.14, 5)` skrevet ut med `np.max`.

Neste: [09 — Python](09-python.md).
