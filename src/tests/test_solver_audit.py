"""numerical audit of the split-step solver.

these tests use mode='linear' and mode='thermal' as a verification harness,
not as Triad regimes. the physical model is always 'full' (P1+P2+P3 together,
never isolated). but propagator unitarity (norm conservation to fp64) and
dt-convergence are mathematically checkable only when the nonlinear and
dissipative terms are switched off, so the audit drives the solver into those
degenerate numerical settings on purpose. no regime, template, compiler config
or .tri program reaches them: see solver._effective_params.
"""
import sys, os
