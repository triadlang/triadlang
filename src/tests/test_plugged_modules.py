...
def test_curriculum_reachable_and_runs():
    ...
    # sigma must never reach the pure-laplacian value 2.0 (P1 stays fractional).
...
def test_memory_reachable_and_runs():
    ...
    # the default params are pillar-complete.
...
def test_qubits_reachable_and_bell_state():
    ...
    # a Bell state has weight only on 00 and 11.
...
def test_qubits_substrate_gates_are_pillar_complete():
    ...
    st = QubitState(1)
    # every named gate must run (all pillars active, none isolated).
