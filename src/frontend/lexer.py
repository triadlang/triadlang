...
def tokenize(src: str, file: str='') -> list[Token]:
    ...
    while i < n:
        ...
        if (c == 'r' and peek(1) == '"') or (c == 'R' and peek(1) == '"'):
            ...
            continue
        if (c == 'r' and peek(1) in 'fF' and peek(2) == '"') or (c == 'R' and peek(1) in 'fF' and peek(2) == '"'):
            sl, sc = (line, col)
            adv()
            adv()
            adv()
        ...
        if c == '-' and peek(1).isdigit():
            ...
            if prev is None or (prev.kind in ('SYMBOL', 'KEYWORD') and prev.value not in (')', ']', '}', 'NUMBER', 'IDENT', 'STRING')):
                sl, sc = (line, col)
                start = i
                adv()
