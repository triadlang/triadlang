# Stdlib — enkle moduler

Importer med `import navn;`. Fullstendige signaturer; ingenting gjemt.
...
import random;
random.seed(0)
random.random()        // flyttall 0..1
random.randint(1, 10)  // int, begge ender inklusive
...
// encode/decode er alias for stringify/parse
```
...
datetime.timestamp()  // epoch-sekunder
datetime.year()  datetime.month()  datetime.day()

import regex;
regex.match("[0-9]+", "abc123")     // false (match = fra start)
regex.search("[0-9]+", "abc123")    // "123"
regex.findall("[0-9]", "a1b22")     // ["1", "2", "2"]
regex.replace("[0-9]", "#", "a1b")  // "a#b"

// plot: matplotlib-manerer (triad.plot er samme modul)
import plot;
plot.plot([0, 1, 2], [0, 1, 4]);
plot.title("parabel");
plot.savefig("ut.png");
import io;
io.print("x");   // print som funksjon (io.input leser stdin)
```
