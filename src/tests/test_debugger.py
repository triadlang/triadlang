...
class TestHookExecution:
    ...
    def test_hook_records_hit(self, dbg):
        ...
        dbg._filename = '<test>'
        dbg._bp_hook(1, {'x': 1})
        assert dbg._hit_count['<test>'][1] == 1

    def test_hook_does_not_break_without_bp(self, dbg):
        dbg._source_lines = ['x = 1']
        dbg._breakpoints = {'<triad>': {5}}
        dbg._step_mode = False
        dbg._filename = '<test>'
        dbg._bp_hook(1, {'x': 1})
        assert dbg._hit_count.get('<test>', {}).get(1, 0) == 1
...
class TestRunSource:
    ...
    def test_run_with_breakpoint_hits(self, dbg):
        source = 'let x = 10\nlet y = 20\nlet z = x + y\n'
        dbg.set_breakpoints([2], source_path='<test>')
        dbg._step_mode = False
        dbg._non_interactive = True
        dbg.run_source(source, '<test>')
        assert 2 in dbg._hit_count['<test>']
