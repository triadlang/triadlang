"""
Tolerant comparator for parity_multi_runtime_3d.

Two 3D substrates (N=8, L=8) evolve through 50 timesteps. Smaller grid
than 2D so tolerances are roughly the same.
"""
import math
import subprocess
import sys
...
NATIVE = REPO / "native" / "c" / "parity" / "parity_multi_runtime_3d"
PY = REPO / "src" / "scripts" / "codegen" / "multi_runtime_3d_to_text.py"
...
def compare_line(py_line, c_line):
    ...
    for i, (a, b) in enumerate(zip(py_toks, c_toks)):
        ...
        if not ok:
            return False, f"float mismatch at tok {i}: {a} vs {b}"
    return True, ""
...
def main():
    ...
    if fails:
        ...
        for f in fails[:10]:
            print(f)
        return 1
    print(f"parity_multi_runtime_3d: PASS ({passes}/{total})")
    return 0
