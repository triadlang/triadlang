...
def build_program():
    ...
    def mk(seed):
        return TriadParams(N=N, L=L, dt=dt, T=0.8, hbar=1.0, m=1.0, omega=1.0, Lambda=-0.3, alpha=0.15, sigma=1.5, Gamma=0.05, f_FDT=0.002, fdt_couple=True, kT=1.0, lam=np.array([-0.03, -0.02, -0.01]), nu=np.array([0.5, 0.1, 0.02]), mode='triad', seed=seed, V_ext='harmonic', D=2)
    rt = MultiRuntime(dt=dt, record_every=4)
    rt.add_substrate('A', mk(11), psi=None)
    rt.add_substrate('B', mk(22), psi=None)
    rt.segments = [Segment(0.0, 0.2, [], None), Segment(0.2, 0.5, [CouplingLink(0, 1, kappa=-0.2, coupling_mode='dc_subtracted')], None), Segment(0.5, 0.8, [CouplingLink(0, 1, kappa=-0.15, coupling_mode='phase_coherent', k_target=0.4), CouplingLink(1, 0, kappa=-0.1, coupling_mode='density')], None)]
    rt.global_t = 0.0
    return rt
