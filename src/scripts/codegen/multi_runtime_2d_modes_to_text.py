...
def build_program():
    ...
    def mk(seed):
        return TriadParams(N=N, L=L, dt=dt, T=0.8, hbar=1.0, m=1.0, omega=1.0, Lambda=-0.3, alpha=0.15, sigma=1.5, Gamma=0.05, f_FDT=0.0, fdt_couple=False, kT=1.0, lam=np.array([0.03]), nu=np.array([0.5]), mode='full', seed=seed, V_ext='harmonic', D=2)
