...
_ML_NAMES = frozenset({
    "ml_tensor", "ml_zeros", "ml_randn", "ml_linear", "ml_forward",
...
def _detect_preamble_needs(module):
    """Scan IR module to detect which preamble sections are needed."""
    needs_solver = False
    needs_ml = False
    needs_ccall = False
    needs_python = False
    def _walk(nodes):
        ...
        for node in nodes:
            ...
            for attr in ("target", "iter", "value", "cond", "expr", "left", "right", "operand"):
...
def _build_preamble(module):
    """Build minimal preamble based on what the IR actually uses."""
    ...
    def _gen_arith_binop(self, left: str, right: str, op: str) -> str:
        ...
        if op == '+':
            ...
            return (f'({{ TriadValue {tmp}_l = ({left}), {tmp}_r = ({right}); '
                    ...
                    f'triad_str_concat(triad_value_to_string({tmp}_l), triad_value_to_string({tmp}_r))}}}} : '
                    f'(({tmp}_l.tag == TRIAD_INT && {tmp}_r.tag == TRIAD_INT) ? '
                    f'TRIAD_INT({tmp}_l.as.ival + {tmp}_r.as.ival) : '
                    f'TRIAD_FLOAT(({tmp}_l.tag==TRIAD_INT ? (double){tmp}_l.as.ival : {tmp}_l.as.fval) + '
                    f'({tmp}_r.tag==TRIAD_INT ? (double){tmp}_r.as.ival : {tmp}_r.as.fval))); }})')
