# Stdlib — plain modules

Import with `import name;`. Full signatures; nothing hidden.
...
random.randint(1, 10)  // int, both ends included
...
datetime.timestamp()  // epoch seconds
datetime.year()  datetime.month()  datetime.day()

import regex;
regex.match("\\d+", "abc123")     // false (match = from start)
regex.search("\\d+", "abc123")    // "123"
regex.findall("\\d", "a1b22")     // ["1", "2", "2"]
regex.replace("\\d", "#", "a1b")  // "a#b"

// plot: matplotlib manners (triad.plot is the same module)
import plot;
plot.plot([0, 1, 2], [0, 1, 4]);
plot.title("parabola");
plot.savefig("out.png");
import io;
io.print("x");   // print as function (io.input reads stdin)
```
