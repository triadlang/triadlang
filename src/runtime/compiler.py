...
class TriadCompiler:
    ...
    def _compile_classify(self, n_classes: int=2, n_features: int=128, depth: int=4, **kwargs) -> CompiledProgram:
        ...
        couplings.append(CouplingConfig(src=f'cls_{n_subs - 1}', dst='cls_0', kappa=-1.5, mode='density'))
        readout = ReadoutConfig(what='triad', fields=('crystallinity', 'k_star', 'peak', 'ipr', 'participation', 'fwhm', 'norm', 'density_pca'))
    ...
    def _compile_generate(self, pattern_type: str='crystal', N: int=128, **kwargs) -> CompiledProgram:
        ...
        if pattern_type == 'crystal':
            ...
            alpha_gen, sigma_gen = (0.15, 1.5)
        subs = [SubstrateConfig(name='gen_0', N=N, Lambda=-8.0 if pattern_type == 'crystal' else -0.5, alpha=alpha_gen, sigma=sigma_gen, nu=nu_gen, lam=lam_split, V_ext=None)]
    ...
    def _compile_couple(self, n_substrates: int=3, topology: str='ring', kappa: float=-3.0, **kwargs) -> CompiledProgram:
        ...
        if topology == 'ring':
            ...
            for i in range(1, n_substrates):
                couplings.append(CouplingConfig(src=f's{i}', dst='s0', kappa=kappa))
                couplings.append(CouplingConfig(src='s0', dst=f's{i}', kappa=kappa))
    ...
    def _compile_sat(self, n_vars: int=3, n_clauses: int=9, **kwargs) -> CompiledProgram:
        N = max(64, n_vars * 8)
        return CompiledProgram(substrates=[SubstrateConfig(name='sat_0', N=N, Lambda=-5.0, alpha=0.1, sigma=1.7, V_ext=None, Gamma=0.02, f_FDT=0.001, nu=(2.0, 0.5, 0.1), lam=(-0.5, -0.3, -0.1))], couplings=[], readout=ReadoutConfig(), T=10.0, metadata={'task': 'sat', 'n_vars': n_vars, 'n_clauses': n_clauses})

def instantiate(program: CompiledProgram, seed: int=0) -> TriadRuntime:
    n_subs = len(program.substrates)
    sub0 = program.substrates[0]
    rt = TriadRuntime(n_substrates=n_subs, regime=sub0.regime, N=sub0.N, coupling=_topology_from_couplings(program.couplings, n_subs), kappa=program.couplings[0].kappa if program.couplings else -3.0, seed=seed, backend=program.backend, readout=program.readout)
    return rt
