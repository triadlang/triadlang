...
from runtime.core.multi_runtime import MultiRuntime, CouplingEdge, Segment, Substrate
from runtime.core.solver import integrate_adaptive, _integrate_steps
from runtime.physics.observables import crystallinity, dominant_wavenumber, peak_density, ipr, participation_ratio, norm as field_norm, fwhm, energy
...
class EquationRuntime:
    ...
    def run_adaptive(self, T: float = 30.0, dt: float = 0.005, stop: str = "adaptive",
        ...
        """Run with adaptive stopping based on convergence observer.

        Parameters
        ----------
        T : float
            Maximum integration time (hard upper bound).
        dt : float
            Time step.
        stop : str
            "adaptive" for convergence-based stopping, "fixed" for regular run.
        risk : float
            Tolerance for convergence certification (mapped to observer tol).
        verbose : bool
            Print progress.
        """
        import time as _time
    ...
    def run_verified(self, T: float = 10.0, dt: float = 0.005, ensemble: int = 5,
        ...
        """Run an ensemble and return results with confidence/disagreement."""
        from stdlib.regimes import resolve_regime
        import time as _time
        t0 = _time.perf_counter()
    ...
    def solve_equilibrium(self, drive: np.ndarray | None = None,
        ...
        """Fixed-point relaxation to equilibrium (B3: C-DEQ style).

        Finds the equilibrium fixed point psi* = S(psi*, y*; drive)
        where S is the full three-pillar propagator with optional drive.

        Parameters
        ----------
        drive : ndarray or None
            External drive potential.  None = no drive.
        tol : float
            Convergence tolerance on observable residual.
        max_iter : int
            Maximum fixed-point iterations.
        anderson_m : int
            Anderson acceleration window (0 = Picard).
        anderson_beta : float
            Anderson damping factor.
        substeps : int
            Sub-steps per fixed-point iteration.
        verbose : bool

        Returns
        -------
        dict with psi_star, y_star, converged, residual, n_iter,
             n_total_steps, residual_history, observables, method.
        """
        from stdlib.regimes import resolve_regime
        import time as _time
        t0 = _time.perf_counter()
    ...
    def equilibrate(self, n_super: int = 50, max_supersteps: int = 60,
        ...
        """Fast equilibrium solve via super-stepping with observable convergence."""
        from stdlib.regimes import resolve_regime
        import time as _time
        t0 = _time.perf_counter()
