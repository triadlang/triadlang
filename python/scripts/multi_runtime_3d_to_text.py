...
def build_program() -> MultiRuntime:
    ...
    def make_params(seed: int) -> TriadParams:
        return TriadParams(N=N, L=L, dt=dt, T=T_settle + T_couple, hbar=1.0, m=1.0, omega=1.0, Lambda=-0.2, alpha=0.0, sigma=2.0, Gamma=0.05, f_FDT=0.0, lam=np.array([0.02]), nu=np.array([0.4]), mode='full', seed=seed, V_ext='harmonic', D=3)
