...
Importa con `import nombre;`. Firmas completas; nada escondido.
...
random.randint(1, 10)  // int, ambos fines incluidos
...
// encode/decode son alias de stringify/parse
```
...
import regex;
regex.match("[0-9]+", "abc123")     // false (match = desde el inicio)
regex.search("[0-9]+", "abc123")    // "123"
regex.findall("[0-9]", "a1b22")     // ["1", "2", "2"]
regex.replace("[0-9]", "#", "a1b")  // "a#b"

// plot: modales matplotlib (triad.plot es lo mismo)
import plot;
plot.plot([0, 1, 2], [0, 1, 4]);
plot.title("parábola");
plot.savefig("salida.png");
import io;
io.print("x");   // print como función (io.input lee stdin)
```
