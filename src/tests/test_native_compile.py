...
_TRI_ENV = os.environ.copy()
_TRI_ENV['PYTHONPATH'] = os.path.join(ROOT, 'src')

_HAS_NATIVE_RT = os.path.isfile(os.path.join(RT_DIR, 'include', 'triad_rt.h'))
pytestmark = pytest.mark.skipif(
    not _HAS_NATIVE_RT,
    reason='native C runtime not present (python-only build); native/c/include/triad_rt.h missing',
)

def _compile_native(src: str) -> str:
    ...
    c_path = bin_path + '.c'
    try:
        r = subprocess.run(TRIAD + ['compile', tri_path, '--native', '-o', bin_path], capture_output=True, text=True, cwd=ROOT, env=_TRI_ENV)
...
def _run_native(bin_path: str, timeout=10) -> str:
    ...
    r = subprocess.run([bin_path], capture_output=True, text=True, timeout=timeout, env=env)
...
def _run_python(src: str) -> str:
    with tempfile.NamedTemporaryFile(suffix='.tri', mode='w', delete=False) as f:
        ...
        tri_path = f.name
    try:
        r = subprocess.run(TRIAD + ['run', tri_path], capture_output=True, text=True, cwd=ROOT, env=_TRI_ENV)
        return r.stdout.strip()
    finally:
        os.unlink(tri_path)
